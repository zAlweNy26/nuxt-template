...
const { class: mainClass, root } = defineProps<{
...
<template>
  <TagsInputRoot v-bind="forwarded" 
    :class="cn(tagsInputVariants({ size, color }), mainClass)">
    ...
    <TagsInputInput v-bind="input" :disabled
      :class="cn('min-h-6 w-min max-w-96 focus:outline-none flex-1 bg-transparent px-1', inputClass)" />
  </TagsInputRoot>
</template>
