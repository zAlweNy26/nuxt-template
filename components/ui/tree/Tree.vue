...
import { type FlattenedItem, type TreeRootEmits, type TreeRootProps, type TreeVirtualizerProps, useForwardPropsEmits } from 'radix-vue'
		...
		<Tree.Virtualizer v-slot="{ item }" :textContent :estimateSize>
			<Tree.Item v-slot="slotProps" :style="{ 'padding-left': `${item.level - 0.5}rem` }" v-bind="item.bind"
				...
				<slot name="item" v-bind="{ ...(item as FlattenedItem<TreeItem<T>>), ...slotProps }">
					<Checkbox v-if="multiple" :checked="slotProps.isIndeterminate ? 'indeterminate' : slotProps.isSelected" class="mr-2"
						tabindex="-1" size="xs" @click.stop @update:checked="slotProps.handleSelect" />
