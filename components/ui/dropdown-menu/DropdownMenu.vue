...
const { class: mainClass, root, trigger, content } = defineProps<{
...
const triggerProps = computed<DropdownMenuTriggerProps>(() => ({ asChild: true, ...trigger }))
const contentProps = computed<DropdownMenuContentProps>(() => ({ sideOffset: 4, ...content }))

const emits = defineEmits<DropdownMenuRootEmits>()

const forwardedRoot = useForwardPropsEmits(() => root ?? {}, emits)
</script>
