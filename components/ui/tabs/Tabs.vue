...
const { root } = defineProps<{
  root?: Omit<TabsRootProps, 'modelValue'>
  loop?: boolean
  forceMount?: boolean
  items: TabItems
...
onMounted(() => {
  model.value = model.value ?? root?.defaultValue
})
</script>

<template>
  <TabsRoot v-model="model" v-bind="root" :class="cn('flex gap-2', { 'flex-col': root?.orientation === 'horizontal' })">
    <TabsList :loop :class="cn(
      ...
      <TabsTrigger v-for="item in items" :key="item.id" :value="item.id" :disabled="item.disabled" :class="cn(
        ...
        root?.orientation === 'vertical' ? 'w-full' : '',
        item.class,
      )">
    ...
    <TabsContent v-for="item in items" :key="item.id" :value="item.id" :forceMount
      :class="cn('ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2', contentClass)">
